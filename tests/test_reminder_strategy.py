...
class TestReminderStrategy(unittest.TestCase):
    ...
    def test_onetime_reminder_triggered_when_past_due(self):
        ...
        result, _ = self.strategy.check(reminders, now)
    ...
    def test_weekly_reminder_dedup_same_minute(self):
        ...
        result1, key1 = self.strategy.check(reminders, now)
        self.assertEqual(len(result1), 1)

        result2, _ = self.strategy.check(reminders, now, last_minute_key=key1)
    ...
    def test_weekly_reminder_triggers_again_next_minute(self):
        ...
        _, key1 = self.strategy.check(reminders, now1)

        # 下週一同時間
        now2 = datetime(2025, 1, 13, 9, 0, 0)
        result, _ = self.strategy.check(reminders, now2, last_minute_key=key1)
        self.assertEqual(len(result), 1)
    ...
    def test_empty_reminders_returns_empty(self):
        result, _ = self.strategy.check([], datetime(2025, 1, 1, 9, 0, 0))
